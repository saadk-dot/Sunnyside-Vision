...
function AdminLogin({ onLogin }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: '#F7F4EE', borderRadius: 16,
        padding: '48px', width: 380, textAlign: 'center'
      }}>
        <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 28, marginBottom: 8 }}>
          Sunnyside Vision
        </div>
        <div style={{ fontSize: 13, color: '#6B6458', marginBottom: 32, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
          Admin Panel
        </div>
        <input
          type="password"
          ...
          style={{
            ...
            border: `1.5px solid ${error ? '#C8873A' : '#DDD8CE'}`,
            background: '#FDFCF9', fontSize: 15, marginBottom: 16,
...
function AdminSidebar({ onLogout }) {
  ...
  return (
    <div style={{
      width: 240, background: '#1A1814', color: '#F7F4EE',
      ...
      <div style={{ padding: '28px 24px', borderBottom: '1px solid #2A2620' }}>
        ...
        <div style={{ fontSize: 11, color: '#6B6458', marginTop: 4, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
          Admin Panel
        </div>
      </div>
      ...
      <div style={{ padding: '16px 12px', borderTop: '1px solid #2A2620' }}>
        <Link to="/" target="_blank" style={{
          ...
          color: '#A09880', fontSize: 13, marginBottom: 8
        }}>
          ↗ View Live Site
        </Link>
        <button onClick={onLogout} style={{
          ...
          color: '#6B6458', fontSize: 13, textAlign: 'left',
          cursor: 'pointer'
        }}>
          Log out
        </button>
  ...
  return (
    <div style={{ padding: '48px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, marginBottom: 40 }}>
        {[
          { num: stats.locations, label: 'Map Locations', to: '/admin/locations', color: '#C8873A' },
          { num: stats.responses, label: 'Survey Responses', to: '/admin/responses', color: '#3D6B5A' },
          { num: stats.gallery, label: 'Gallery Items', to: '/admin/gallery', color: '#5B7FA6' },
        ].map(s => (
          <Link key={s.label} to={s.to} style={{
            background: 'white', borderRadius: 12, padding: '28px',
            border: '1px solid #DDD8CE', textDecoration: 'none', color: '#1A1814',
      ...
      <Link to="/admin/locations" style={{
        display: 'inline-flex', padding: '13px 28px', borderRadius: 8,
        background: '#1A1814', color: '#F7F4EE', fontWeight: 600, fontSize: 15
      }}>
        Manage Locations →
      </Link>
    </div>
        ...
        {locations.map(loc => (
          <div key={loc.id} style={{
            ...
            {loc.location_images?.[0]?.image_url ? (
              <img src={loc.location_images[0].image_url} alt={loc.name}
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                ...
                <span style={{ fontSize: 12, color: '#6B6458', background: '#F0EDE6', padding: '2px 8px', borderRadius: 12 }}>
                  {loc.survey_questions?.length || 0} questions
                </span>
              </div>
              <p style={{ fontSize: 13, color: '#6B6458', lineHeight: 1.5 }}>{loc.description}</p>
              <p style={{ fontSize: 12, color: '#A09880', marginTop: 4 }}>
                📍 {loc.latitude?.toFixed(4)}, {loc.longitude?.toFixed(4)}
              </p>
            </div>
            <div style={{ display: 'flex', gap: 10, flexShrink: 0 }}>
              <button onClick={() => navigate(`/admin/locations/${loc.id}`)} style={{
                padding: '8px 18px', borderRadius: 8,
                border: '1.5px solid #DDD8CE', background: 'none',
                fontSize: 13, fontWeight: 500, cursor: 'pointer'
              }}>Edit</button>
              <button onClick={() => deleteLocation(loc.id)} style={{
                padding: '8px 18px', borderRadius: 8,
...
function AddLocationForm({ onSave, onCancel }) {
  ...
  return (
    <div style={{
      background: 'white', borderRadius: 12, padding: '28px',
      border: '1.5px solid #C8873A', marginBottom: 24
    }}>
      <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 24, marginBottom: 20 }}>New Location</h3>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        {[
          { label: 'Location Name', key: 'name', placeholder: 'e.g. Bliss & 46th — The Gateway' },
          { label: 'Color (hex)', key: 'color', placeholder: '#C8873A' },
          { label: 'Latitude', key: 'latitude', placeholder: '40.7440' },
          { label: 'Longitude', key: 'longitude', placeholder: '-73.9226' },
        ].map(f => (
          <div key={f.key}>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        <textarea
          ...
          style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1.5px solid #DDD8CE', fontSize: 14, minHeight: 80, resize: 'vertical', outline: 'none' }}
        />
      </div>
      <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
        <button onClick={save} disabled={saving} style={{ padding: '11px 28px', borderRadius: 8, background: '#1A1814', color: 'white', border: 'none', fontWeight: 600, fontSize: 14 }}>
...
// ── Location Detail (edit + questions + image) ─────────────────────────────
function AdminLocationDetail() {
  ...
  async function deleteImage(imgId, imgUrl) {
  ...
  return (
    <div style={{ padding: '48px' }}>
      ...
      <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 40, marginBottom: 32 }}>
        Edit: {loc.name}
      </h1>
      ...
      <div style={{ background: 'white', borderRadius: 12, padding: '28px', border: '1px solid #DDD8CE', marginBottom: 24 }}>
        ...
        <button onClick={saveLocation} disabled={saving} style={{ marginTop: 20, padding: '11px 28px', borderRadius: 8, background: '#1A1814', color: 'white', border: 'none', fontWeight: 600 }}>
      ...
      <div style={{ background: 'white', borderRadius: 12, padding: '28px', border: '1px solid #DDD8CE', marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          ...
          <button onClick={() => fileRef.current?.click()} disabled={uploading} style={{
            padding: '9px 20px', borderRadius: 8, background: '#C8873A', color: 'white', border: 'none', fontWeight: 600, fontSize: 13
          }}>
        ...
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          {images.map(img => (
            <div key={img.id} style={{ position: 'relative' }}>
              ...
              <button onClick={() => deleteImage(img.id, img.image_url)} style={{
          ...
          {images.length === 0 && <p style={{ color: '#A09880', fontSize: 14, fontStyle: 'italic' }}>No photos yet. Upload one above.</p>}
        </div>
      </div>
      ...
      <div style={{ background: 'white', borderRadius: 12, padding: '28px', border: '1px solid #DDD8CE' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          ...
          <button onClick={() => setShowAddQ(true)} style={{
            padding: '9px 20px', borderRadius: 8, background: '#3D6B5A', color: 'white', border: 'none', fontWeight: 600, fontSize: 13
          }}>+ Add Question</button>
        </div>
        ...
        {questions.map((q, i) => (
          <QuestionRow key={q.id} question={q} index={i} onDelete={() => deleteQuestion(q.id)} onUpdate={load} />
        ))}

        {questions.length === 0 && !showAddQ && (
          <p style={{ color: '#A09880', fontSize: 14, fontStyle: 'italic' }}>No questions yet. Add your first question above.</p>
        )}
      </div>
    </div>
  )
    ...
    <div style={{ background: '#F7F4EE', borderRadius: 10, padding: '20px', marginBottom: 12 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 0.5fr', gap: 12, marginBottom: 12 }}>
        <div>
          ...
          <input value={form.question_text} onChange={e => setForm(p => ({...p, question_text: e.target.value}))}
            style={inputStyle} />
        </div>
        <div>
          <label style={labelStyle}>Type</label>
          <select value={form.question_type} onChange={e => setForm(p => ({...p, question_type: e.target.value}))} style={inputStyle}>
        ...
        <div>
          ...
          <input type="number" value={form.order_num} onChange={e => setForm(p => ({...p, order_num: parseInt(e.target.value)}))} style={inputStyle} />
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={save} disabled={saving} style={{ padding: '8px 20px', borderRadius: 7, background: '#1A1814', color: 'white', border: 'none', fontWeight: 600, fontSize: 13 }}>
          {saving ? 'Saving...' : 'Save'}
        </button>
    ...
    <div style={{ background: '#F7F4EE', borderRadius: 10, padding: '20px', marginBottom: 20, border: '1.5px solid #3D6B5A' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12, marginBottom: 12 }}>
        <div>
          <label style={labelStyle}>Question Text</label>
          <input value={form.question_text} onChange={e => setForm(p => ({...p, question_text: e.target.value}))}
            placeholder="What do you imagine here?" style={inputStyle} />
        </div>
        <div>
      ...
      {(form.question_type === 'choice' || form.question_type === 'multi') && (
        <div style={{ marginBottom: 12 }}>
          <label style={labelStyle}>Options (comma separated)</label>
          <input value={form.options} onChange={e => setForm(p => ({...p, options: e.target.value}))}
            placeholder="Green space, Seating area, Public art, Market stalls" style={inputStyle} />
        </div>
      )}
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={save} disabled={saving} style={{ padding: '9px 24px', borderRadius: 7, background: '#3D6B5A', color: 'white', border: 'none', fontWeight: 600, fontSize: 13 }}>
...
function AdminGallery() {
  ...
  async function toggleApprove(id, current) {
    await supabase.from('gallery').update({ approved: !current }).eq('id', id)
    load()
  }
        ...
        {items.map(item => (
          <div key={item.id} style={{ background: 'white', borderRadius: 12, overflow: 'hidden', border: '1px solid #DDD8CE' }}>
            <img src={item.image_url} alt="" style={{ width: '100%', height: 180, objectFit: 'cover', display: 'block' }}
              onError={e => e.target.src = `https://picsum.photos/seed/${item.id}/400/200`} />
            <div style={{ padding: '14px' }}>
              <div style={{ fontWeight: 500, fontSize: 14, marginBottom: 4 }}>{item.locations?.name}</div>
              <div style={{ fontSize: 12, color: '#A09880', marginBottom: 12 }}>
                {new Date(item.created_at).toLocaleDateString()}
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => toggleApprove(item.id, item.approved)} style={{
                  flex: 1, padding: '7px', borderRadius: 7, fontSize: 12, fontWeight: 600,
                  border: 'none', background: item.approved ? '#E8F5E9' : '#FFF3E0',
                  color: item.approved ? '#2E7D32' : '#E65100', cursor: 'pointer'
                }}>
                  {item.approved ? '✓ Approved' : '○ Hidden'}
                </button>
                <button onClick={() => deleteItem(item.id)} style={{
                  padding: '7px 12px', borderRadius: 7, border: '1.5px solid #FFCCCC',
          ...
          <div key={r.id} style={{ background: 'white', borderRadius: 10, padding: '18px 22px', border: '1px solid #DDD8CE' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {Object.entries(r.answers || {}).map(([k, v]) => (
                <div key={k} style={{ fontSize: 12, background: '#F0EDE6', padding: '4px 10px', borderRadius: 6, color: '#4A4438' }}>
...
// Shared styles
const labelStyle = { display: 'block', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#6B6458', marginBottom: 5 }
const inputStyle = { width: '100%', padding: '9px 12px', borderRadius: 8, border: '1.5px solid #DDD8CE', fontSize: 14, outline: 'none', background: 'white', color: '#1A1814' }
